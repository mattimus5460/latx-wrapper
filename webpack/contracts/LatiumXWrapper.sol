...
contract DepositSlot {
    address public constant LATX = 0x2f85E502a988AF76f7ee6D83b7db8d6c0A823bf9;
    ...
    function collect() onlyWrapper {
        uint amount = TokenInterface(LATX).balanceOf(this);
        if (amount == 0) throw;

        TokenInterface(LATX).transfer(wrapper, amount);
    }
}
...
contract LatiumXTokenWrapped is Token {
    ...
    function withdrawLATX(uint amount) internal {
        ...
        TokenInterface(LATX).transfer(msg.sender, amount);
    }
}